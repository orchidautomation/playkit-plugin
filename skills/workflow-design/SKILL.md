...
### `brainstorm_play`
        ...
        Args:
            ...
            industry, goal, team_role, company_stage, stage, target_audience:
                Optional structured fields accepted when an agent decomposes the
                use case instead of sending one string.
...
<!-- pluxx:custom:start -->
When requesting `design_clay(output_format="build_spec")`, expect a `tables` array with multi-table connections, status columns, and credit estimates. Pass that wrapper directly to `clay_build_table(table_spec_json=...)`; the build tool now handles both single-table specs and the full `tables` wrapper.

For email/phone fallback flows, prefer native Clay waterfall formula columns in the build spec: `type="formula"`, `formulaType="waterfall"`, `waterfallType`, optional `truncateValue`, and `formulaWaterfall` steps. The provider actions should be separate upstream columns.
<!-- pluxx:custom:end -->
